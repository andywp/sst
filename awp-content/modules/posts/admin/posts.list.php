...
if($this->_GET('cat') && $this->_GET('cat')!='any'){
	$sqlCond .= " and post_section='".$this->_GET('cat')."' ";
}
...
$query		= "select post_id,post_section,".$fTitle.",posts_author,created_date,published_date,publish from ".$tableName." where post_type='post'".$addWhere." order by post_id desc";
...
if(in_array($this->admin('group_id'),$publisAccess)){
	...
	$data['Section'] ='post_section.select.refTable:'.$this->table_prefix.'category where category_type=\'main\',category_name(category_id).width="130"';
