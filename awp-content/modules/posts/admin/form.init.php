...
// add new category
$arrCategory = $this->db->getAll("select category_id,category_name from ".$this->table_prefix."category where category_type='post'");
$arrOption	 = '';
...
if(in_array($this->admin('group_id'),$publisAccess)){
	...
	$params[] = $this->form->input->select('Section', 'add_post_section',$arrSection,$multiple=false);
	$params[] = $this->form->input->text('Author', 'add_posts_author',30,$multilang=false, $this->admin('name'));
